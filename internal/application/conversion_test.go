...
func TestConversionIterate(t *testing.T) {
	...
	conv := NewConversion(stubAffineRepo{}, stubTransformRepo{}, rnd, sink)
...
func TestConversionStepValidation(t *testing.T) {
	...
	rnd := rand.New(rand.NewSource(1))
	conv := NewConversion(stubAffineRepo{}, stubTransformRepo{}, rnd, nil)

	_, err := conv.Step(false, domain.Point{})
	assert.NoError(t, err)

	convEmpty := NewConversion(stubAffineRepo{}, stubTransformRepo{}, rnd, nil)

	convEmpty.affRepo = stubAffineRepoLenZero{}
	_, err = convEmpty.Step(true, domain.Point{})
	assert.ErrorContains(t, err, domain.ErrGenerate.Error())
}
